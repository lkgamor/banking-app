...
const findResource = () => {
	...
	//BUILD QUERY URL
	const URL = `${CONTEXT}api/v1/withdrawals/search?q=${TRANSACTION_ID}`;

	//PASS URL TO FUNCTION
	getWithdrawalsList(URL);
};
...
const findAllResources = () => {
	...
	//BUILD QUERY URL
	const url = `${CONTEXT}api/v1/withdrawals/page`;
	...
	$.ajax({
		...
		success: (response) => {			
			...
			if (totalElements > 0) {	
				...
				const MSG = `NO WITHDRAWALS ${employeeMsg} ${periodMsg} ${searchParamMsg}`;
				...
				const noResultsElement = `<div class="void">
											...
											<img class="void__image" alt="No Withdrawals" src="${CONTEXT}images/transactions.svg">
										 </div>`;
...
const buildTable = withdrawals => {
	...
	withdrawals.forEach((withdrawal) => {
		...
		const row = `<tr class="transaction__record withdrawal__record" id=${withdrawalId}>
						<td class="transaction__number">
							<a href="${CONTEXT}withdrawals/${withdrawalId}" title="#${withdrawalId}">#${withdrawalId}</a>
						</td>
						<td class="transaction__account-number">
							<a href="${CONTEXT}accounts/${withdrawalAccountNumber}" title="${withdrawalAccountNumber}">#${withdrawalAccountNumber}</a>
						</td>
